(function($) {
    ...
    function addDragOntoDiv(height, top, extraClass, containerLeft) {
        ...
        $('<div class="kem-drag-onto ' + extraClass + '" />').css({
            ...
            var newDropTarget;
            var oldRank;
            if($(event.delegateTarget).hasClass('kem-drag-onto-top')) {
                newDropTarget = $('<div class="kem-rank" />').prependTo(elems.first().parent());
                oldRank = $('.kem-dragging').closest('.kem-rank');
                // Bug where sometimes elements get drug together somehow?
